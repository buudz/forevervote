-- Make poll context optional while preserving the 1500-character safety limit.
...
alter table public.polls add constraint polls_description_check
  check (length(description) <= 1500 and description !~ '[<>]');
...
begin
  if TG_OP = 'UPDATE' then
    ...
    if old.status <> 'draft' and (
      ...
      raise exception 'Posted poll title and context cannot be changed' using errcode = '23514';
    end if;
  ...
  if new.status <> 'draft' then
    ...
    if length(new.description) > 1500 or new.description ~ '[<>]' then
      raise exception 'Poll context must be at most 1500 characters and cannot contain angle brackets' using errcode = '23514';
    end if;
...
comment on column public.polls.description is
  'Optional poll context. May be empty; maximum 1500 characters.';
