...
begin
  ...
  if current_poll.title is distinct from clean_title then
    fields := array_append(fields, 'title');
  end if;
  if coalesce(current_poll.description, '') is distinct from clean_description then
    fields := array_append(fields, 'rationale');
  end if;
  if current_poll.category is distinct from p_category then
    fields := array_append(fields, 'category');
  end if;
  if current_poll.allow_multiple_answers is distinct from coalesce(p_allow_multiple_answers, false) then
    fields := array_append(fields, 'voting_mode');
  ...
  values (
    current_poll.id,
    current_poll.slug,
    current_poll.creator_id,
    nullif(p_editor_battlenet_account_id, ''),
    coalesce(nullif(btrim(p_editor_battletag), ''), 'Poll creator'),
    'creator',
    (select h.old_title from (select clean_title as new_title) x cross join lateral (select current_poll.title as old_title) h),
    clean_title,
    '',
    clean_description,
    null,
...
-- Recreate creator_edit_poll with correct before/after values captured before update.
create or replace function public.creator_edit_poll(
  p_poll_id uuid,
  p_user_id uuid,
  p_title text,
